...
This compiles the client into a jar located at `./target/anprcloud-client-1.0-SNAPSHOT.jar`. Note that this jar does not include any dependencies.
...
```perl
dependencies {
    compile 'hu.arh:anprcloud-client:1.0-SNAPSHOT'
}
```
...
This will compile the client and package all dependencies into a jar located at `./target/anprcloud-client-1.0-SNAPSHOT.jar`.

# Developer Guide

## Setting the configuration
To set the service endpoint configuration, use the ANPRCloudConfig class. You can obtain an instance of the ANPRCloudConfig via a static factory method.
...
```java
ANPRCloudServiceClientBuilder builder = ANPRCloudService.builder();
```
...
```java
ANPRCloudService client = ANPRCloudService.builder()
...
## Making requests
After a client is configured and created, you can make a request to the service. A method on the client interface (`ANPRCloudService`) is created for all actions (resource + method) in your API.
...
Service exceptions and client exceptions can be handled separately. Any exceptions modeled in the API will be a subtype of ANPRCloudServiceException.
```java
try {
    client.process(...);
   ...
   // All service exceptions will extend from ANPRCloudServiceException.
   // Any unknown or unmodeled service exceptions will be represented as a ANPRCloudServiceException.
